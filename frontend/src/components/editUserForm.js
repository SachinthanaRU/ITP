...
function UserEdit() {
  ...
  const [checked, setChecked] = useState(false);
    ...
    useEffect(() => {const fetchUsers = async () => {
        try {
          const response = await axios.get(`http://localhost:4000/user/${params.id}`); // Replace with the appropriate API endpoint
  ...
  const handleSubmit = (event) => {
    ...
    console.log(formData);
     ...
     axios
      .put(`http://localhost:4000/user/edit/${params.id}`, formData)
      .then(() => {
